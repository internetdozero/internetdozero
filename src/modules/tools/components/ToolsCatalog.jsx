...
const catalogItems = [
  ...
  { slugPt: 'conversor-de-arquivos', slugEn: 'file-converter', titlePt: 'Conversor de arquivos', titleEn: 'File converter', descPt: 'Converta arquivos entre extensões sem envio a servidores.', descEn: 'Convert files between extensions without server uploads.', Icon: ArrowLeftRight },
...
export function ToolsCatalog({ onNavigate, lang = 'pt' }) {
  ...
  return (
    <main className="mx-auto max-w-7xl px-6 py-12 sm:py-16">
      <button type="button" onClick={() => onNavigate('/')} className="mb-8 inline-flex w-fit items-center gap-2 font-mono text-xs text-zinc-500 transition-colors hover:text-stone-500">
        <ArrowLeft className="h-3.5 w-3.5" />
        {isEn ? 'Back to home' : 'Voltar ao início'}
      </button>
      <div className="mb-6 flex h-12 w-12 items-center justify-center rounded-2xl border border-stone-500/20 bg-stone-500/10 text-stone-500">
      ...
      <div className="mt-10 grid gap-4 sm:grid-cols-2 lg:grid-cols-3">
        {catalogItems.map((item) => {
          ...
          return (
            <button key={item.slugPt} type="button" onClick={() => onNavigate(path)} className="group rounded-2xl border border-zinc-200 bg-white p-5 text-left transition-colors hover:border-stone-500/50 dark:border-zinc-800 dark:bg-zinc-900/60 cursor-pointer">
              <div className="flex items-start justify-between gap-4">
                <span className="flex h-11 w-11 items-center justify-center rounded-xl border border-stone-500/20 bg-stone-500/10 text-stone-500"><Icon className="h-5 w-5" /></span>
                <ArrowRight className="h-4 w-4 text-zinc-400 transition-transform group-hover:translate-x-1 group-hover:text-stone-500" />
              </div>
              ...
              <span className="mt-4 inline-block font-mono text-[11px] text-stone-500">{path}</span>
            </button>
          );
        })}
      </div>
