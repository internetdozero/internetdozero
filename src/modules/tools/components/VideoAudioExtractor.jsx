...
export function VideoAudioExtractor({ lang = 'pt' }) {
  ...
  const extract = async () => {
    ...
    try {
      await new Promise((resolve, reject) => { video.onloadedmetadata = resolve; video.onerror = () => reject(new Error(isEn ? 'This video could not be read.' : 'Não foi possível ler este vídeo.')); });
      const stream = video.captureStream?.() || video.mozCaptureStream?.();
      if (!stream) throw new Error(isEn ? 'This browser cannot extract audio from video.' : 'Este navegador não consegue extrair áudio de vídeo.');
      video.ontimeupdate = () => setProgress(video.duration ? Math.min(99, Math.round((video.currentTime / video.duration) * 100)) : 0);
      const audioTracks = stream.getAudioTracks();
      if (!audioTracks.length) throw new Error(isEn ? 'This video has no audio track.' : 'Este vídeo não tem faixa de áudio.');
      if (!window.MediaRecorder) throw new Error(isEn ? 'This browser cannot record extracted audio.' : 'Este navegador não consegue gravar o áudio extraído.');
      ...
      const chunks = [];
      const recorded = new Promise((resolve) => { recorder.ondataavailable = (event) => event.data.size && chunks.push(event.data); recorder.onstop = () => resolve(new Blob(chunks, { type: recorder.mimeType })); });
      video.onended = () => { setProgress(100); recorder.stop(); }; recorder.start(250); await video.play();
      const audioBuffer = await decodeRecordedAudio(await recorded);
      const blob = encodeWav(audioBuffer);
      setResult({ url: URL.createObjectURL(blob), name: `${file.name.replace(/\.[^.]+$/, '')}-audio.wav`, size: blob.size, duration: audioBuffer.duration });
    } catch (extractError) { if (video.readyState > 0) video.pause(); setProgress(0); setError(extractError.message || (isEn ? 'Audio extraction failed.' : 'Não foi possível extrair o áudio.')); } finally { setProcessing(false); }
  };
  ...
  return (
    <section className="mt-10 overflow-hidden rounded-3xl border border-zinc-200 bg-white dark:border-zinc-800 dark:bg-zinc-900/60" aria-labelledby="video-audio-extractor-title">
      ...
      <div className="grid gap-6 p-5 sm:p-7 lg:grid-cols-[1fr_280px]">
        ...
        <div className="space-y-5 rounded-2xl border border-zinc-200 bg-zinc-50 p-4 dark:border-zinc-800 dark:bg-zinc-950/50"><div className="flex items-start gap-3"><ShieldCheck className="mt-0.5 h-5 w-5 shrink-0 text-emerald-500" /><p className="text-xs leading-relaxed text-zinc-500 dark:text-zinc-400">{isEn ? 'Your video stays in this browser. Extraction takes about as long as the video.' : 'Seu vídeo fica neste navegador. A extração leva aproximadamente o tempo do vídeo.'}</p></div>{processing && <div className="rounded-xl border border-emerald-500/20 bg-emerald-500/5 p-3" role="status" aria-live="polite"><div className="flex items-center justify-between gap-3 text-xs font-semibold text-emerald-600 dark:text-emerald-400"><span>{isEn ? 'Extracting audio…' : 'Extraindo áudio…'}</span><span className="font-mono tabular-nums">{progress}%</span></div><div className="mt-2 h-2 overflow-hidden rounded-full bg-emerald-500/15"><div className="h-full rounded-full bg-emerald-500 transition-[width] duration-300" style={{ width: `${progress}%` }} /></div><p className="mt-2 text-[11px] text-zinc-500">{isEn ? 'Keep this tab open until it finishes.' : 'Mantenha esta aba aberta até terminar.'}</p></div>}<div className="border-t border-zinc-200 pt-5 dark:border-zinc-800"><p className="mb-3 font-mono text-[11px] font-bold uppercase tracking-[0.16em] text-zinc-500">2. {isEn ? 'Create audio file' : 'Gere o arquivo de áudio'}</p><button type="button" disabled={!file || processing || Boolean(result)} onClick={extract} className="inline-flex w-full items-center justify-center gap-2 rounded-xl bg-emerald-500 px-4 py-3 text-sm font-bold text-zinc-950 transition-colors hover:bg-emerald-400 focus-visible:outline-none focus-visible:ring-2 focus-visible:ring-emerald-500 disabled:cursor-not-allowed disabled:opacity-40">{processing ? (isEn ? 'Extracting…' : 'Extraindo…') : result ? (isEn ? 'Audio created' : 'Áudio criado') : (isEn ? 'Extract audio' : 'Extrair áudio')}</button>{result && <><p className="mt-3 text-xs text-zinc-500">WAV · {Math.round(result.duration)}s · {(result.size / 1024 / 1024).toFixed(2)} MB</p><a href={result.url} download={result.name} className="mt-3 flex w-full items-center justify-center gap-2 rounded-xl border border-emerald-500/40 px-4 py-3 text-sm font-bold text-emerald-600 transition-colors hover:bg-emerald-500/10 focus-visible:outline-none focus-visible:ring-2 focus-visible:ring-emerald-500 dark:text-emerald-400"><Download className="h-4 w-4" />{isEn ? 'Download WAV' : 'Baixar WAV'}</a></>}</div></div>
      </div>
    </section>
  );
}
