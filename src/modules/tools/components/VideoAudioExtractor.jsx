...
export function VideoAudioExtractor({ lang = 'pt' }) {
  ...
  const selectFile = (nextFile) => {
    ...
    setError(''); setFile(nextFile); setVideoUrl(URL.createObjectURL(nextFile)); setResult(null);
  };
  ...
  const extract = async () => {
    ...
    try {
      ...
      video.onended = () => recorder.stop(); recorder.start(250); await video.play();
    ...
    } catch (extractError) { if (video.readyState > 0) video.pause(); setError(extractError.message || (isEn ? 'Audio extraction failed.' : 'Não foi possível extrair o áudio.')); } finally { setProcessing(false); }
  };
