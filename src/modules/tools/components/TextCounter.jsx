    ...
    <section className="mt-10 overflow-hidden rounded-3xl border border-zinc-200 bg-white dark:border-zinc-800 dark:bg-zinc-900/60" aria-labelledby="text-counter-title">
      ...
      <div className="grid gap-6 p-5 sm:p-7 lg:grid-cols-[1fr_280px]">
        ...
        <div className="rounded-2xl border border-zinc-200 bg-zinc-50 p-4 dark:border-zinc-800 dark:bg-zinc-950/50">
          <div className="mb-5 flex items-center gap-3"><FileText className="h-5 w-5 text-emerald-500" /><p className="font-mono text-[11px] font-bold uppercase tracking-[0.16em] text-zinc-500">{isEn ? 'Live stats' : 'Contagem ao vivo'}</p></div>
          <dl className="grid grid-cols-2 gap-3">
            {[[isEn ? 'Words' : 'Palavras', metrics.words], [isEn ? 'Characters' : 'Caracteres', metrics.characters], [isEn ? 'No spaces' : 'Sem espaços', metrics.withoutSpaces], [isEn ? 'Lines' : 'Linhas', metrics.lines]].map(([label, value]) => <div key={label} className="rounded-xl border border-zinc-200 bg-white p-3 dark:border-zinc-800 dark:bg-zinc-900"><dt className="text-[11px] text-zinc-500">{label}</dt><dd className="mt-1 font-mono text-xl font-bold text-zinc-900 dark:text-white">{value}</dd></div>)}
          </dl>
          <div className="mt-3 rounded-xl border border-emerald-500/20 bg-emerald-500/5 p-3"><dt className="text-[11px] text-zinc-500">{isEn ? 'Reading time' : 'Tempo de leitura'}</dt><dd className="mt-1 font-mono text-lg font-bold text-emerald-600 dark:text-emerald-400">{formatReadingTime(metrics.words)}</dd></div>
