    ...
    <section className="mt-10 overflow-hidden rounded-3xl border border-zinc-200 bg-white dark:border-zinc-800 dark:bg-zinc-900/60" aria-labelledby="password-generator-title">
      <div className="border-b border-zinc-200 p-5 dark:border-zinc-800 sm:p-7">
        <div className="flex items-start justify-between gap-4">
          <div>
            <p className="font-mono text-[11px] font-bold uppercase tracking-[0.2em] text-stone-500">gerador local</p>
      ...
      <div className="grid gap-6 p-5 sm:p-7 lg:grid-cols-[1fr_280px]">
        <div>
          <label htmlFor="generated-password" className="font-mono text-[11px] font-bold uppercase tracking-[0.16em] text-stone-500">{isEn ? 'Your password' : 'Sua senha'}</label>
          <div className="mt-3 flex min-h-28 items-center justify-center rounded-2xl border border-stone-500/30 bg-stone-500/5 p-5 text-center">
          ...
          <button type="button" onClick={copyPassword} className="mt-3 inline-flex w-full items-center justify-center gap-2 rounded-xl border border-stone-500/40 px-4 py-3 text-sm font-bold text-stone-600 transition-colors hover:bg-stone-500/10 focus-visible:outline-none focus-visible:ring-2 focus-visible:ring-stone-500 dark:text-stone-400">
            {copied ? <Check className="h-4 w-4" /> : <Clipboard className="h-4 w-4" />}{copied ? (isEn ? 'Copied' : 'Copiada') : (isEn ? 'Copy password' : 'Copiar senha')}
          </button>
          <p className="mt-3 flex items-center gap-2 text-xs text-zinc-500"><KeyRound className="h-3.5 w-3.5 text-stone-500" />{isEn ? 'Generated only in this browser.' : 'Gerada somente neste navegador.'}</p>
        </div>

        <div className="space-y-5 rounded-2xl border border-zinc-200 bg-zinc-50 p-4 dark:border-zinc-800 dark:bg-zinc-950/50">
          <label className="block text-xs font-semibold text-zinc-600 dark:text-zinc-400">{isEn ? 'Length' : 'Tamanho'} <span className="float-right font-mono text-stone-500">{length}</span><input type="range" min="8" max="64" value={length} onChange={(event) => changeLength(event.target.value)} className="mt-3 w-full accent-stone-500" /></label>
          <fieldset className="space-y-3">
            <legend className="font-mono text-[11px] font-bold uppercase tracking-[0.16em] text-zinc-500">{isEn ? 'Include' : 'Incluir'}</legend>
            {Object.keys(characterSets).map((key) => <label key={key} className="flex items-center gap-3 text-xs text-zinc-600 dark:text-zinc-400"><input type="checkbox" checked={options[key]} onChange={() => toggleOption(key)} className="h-4 w-4 accent-stone-500" />{labels[key]}</label>)}
          </fieldset>
          <button type="button" onClick={regenerate} className="inline-flex w-full items-center justify-center gap-2 rounded-xl bg-stone-500 px-4 py-3 text-sm font-bold text-zinc-950 transition-colors hover:bg-stone-400 focus-visible:outline-none focus-visible:ring-2 focus-visible:ring-stone-500"><RefreshCw className="h-4 w-4" />{isEn ? 'Generate another' : 'Gerar outra'}</button>
        </div>
      </div>
