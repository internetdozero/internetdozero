    ...
    <div className="min-w-0 rounded-2xl border border-zinc-200 bg-zinc-50 dark:border-zinc-800 dark:bg-zinc-950/50">
      <div className="flex items-center justify-between gap-3 border-b border-zinc-200 px-4 py-3 dark:border-zinc-800">
        ...
        <>
          ...
          <button type="button" onClick={() => inputRef.current?.click()} className="inline-flex items-center gap-1.5 text-xs font-semibold text-zinc-500 transition-colors hover:text-stone-500 focus-visible:outline-none focus-visible:ring-2 focus-visible:ring-stone-500">
            <FolderOpen className="h-3.5 w-3.5" />{isEn ? 'Open file' : 'Abrir arquivo'}
          </button>
        </>
      </div>
      <textarea id={`diff-${side}`} value={value} onChange={(event) => onChange(event.target.value)} spellCheck="false" placeholder={left ? (isEn ? 'Paste the original text here…' : 'Cole o texto original aqui…') : (isEn ? 'Paste the changed version here…' : 'Cole a versão alterada aqui…')} className="min-h-[20rem] w-full resize-y bg-transparent p-4 font-mono text-sm leading-6 text-zinc-800 outline-none placeholder:text-zinc-400 focus-visible:ring-2 focus-visible:ring-inset focus-visible:ring-stone-500/40 dark:text-zinc-200 dark:placeholder:text-zinc-600" />
    </div>
  );
}
  ...
  return (
    <section className="mt-10 overflow-hidden rounded-3xl border border-zinc-200 bg-white dark:border-zinc-800 dark:bg-zinc-900/60" aria-labelledby="diff-checker-title">
      <div className="border-b border-zinc-200 p-5 dark:border-zinc-800 sm:p-7">
        <div className="flex items-start justify-between gap-4"><div><p className="font-mono text-[11px] font-bold uppercase tracking-[0.2em] text-stone-500">{isEn ? 'local comparison' : 'comparação local'}</p><h2 id="diff-checker-title" className="mt-2 font-mono text-xl font-bold text-zinc-900 dark:text-white sm:text-2xl">{isEn ? 'Text diff checker' : 'Comparador de texto'}</h2><p className="mt-2 text-sm leading-relaxed text-zinc-500 dark:text-zinc-400">{isEn ? 'See what changed between two texts or code files.' : 'Veja o que mudou entre dois textos ou arquivos de código.'}</p></div><ShieldCheck className="mt-1 h-5 w-5 shrink-0 text-stone-500" aria-label={isEn ? 'Local processing' : 'Processamento local'} /></div>
      </div>
      <div className="space-y-5 p-5 sm:p-7">
        ...
        <div className="flex flex-wrap items-center justify-between gap-3 rounded-2xl border border-zinc-200 bg-zinc-50 px-4 py-3 dark:border-zinc-800 dark:bg-zinc-950/50">
          <div className="flex flex-wrap gap-x-4 gap-y-2 text-xs"><span className="text-stone-600 dark:text-stone-400">+ {stats.added} {isEn ? 'added' : 'adicionadas'}</span><span className="text-red-500">− {stats.removed} {isEn ? 'removed' : 'removidas'}</span><span className="text-zinc-500">{changed} {isEn ? 'changed' : 'alteradas'}</span></div>
          <button type="button" onClick={() => { setLeft(''); setRight(''); setFileError(''); }} disabled={!left && !right} className="inline-flex items-center gap-1.5 text-xs font-semibold text-zinc-500 transition-colors hover:text-red-500 disabled:cursor-not-allowed disabled:opacity-40 focus-visible:outline-none focus-visible:ring-2 focus-visible:ring-stone-500"><Trash2 className="h-3.5 w-3.5" />{isEn ? 'Clear' : 'Limpar'}</button>
        </div>
        ...
        {(left || right) && !tooLarge && <div className="overflow-hidden rounded-2xl border border-zinc-200 dark:border-zinc-800"><div className="flex items-center gap-2 border-b border-zinc-200 px-4 py-3 dark:border-zinc-800"><FileDiff className="h-4 w-4 text-stone-500" /><p className="font-mono text-[10px] font-bold uppercase tracking-[0.16em] text-zinc-500">{isEn ? 'Line-by-line result' : 'Resultado linha a linha'}</p></div><div className="max-h-[28rem] overflow-auto font-mono text-xs">{rows.map((row, index) => <div key={`${row.type}-${index}`} className={`grid grid-cols-2 border-b border-zinc-200/70 last:border-0 dark:border-zinc-800/70 ${row.type === 'added' ? 'bg-stone-500/10' : row.type === 'removed' ? 'bg-red-500/10' : ''}`}><div className={`min-w-0 whitespace-pre-wrap break-words border-r border-zinc-200/70 px-3 py-2 dark:border-zinc-800/70 ${row.type === 'removed' ? 'text-red-600 dark:text-red-400' : 'text-zinc-600 dark:text-zinc-400'}`}>{row.left === null ? <span className="text-zinc-400">—</span> : row.left}</div><div className={`min-w-0 whitespace-pre-wrap break-words px-3 py-2 ${row.type === 'added' ? 'text-stone-700 dark:text-stone-400' : 'text-zinc-600 dark:text-zinc-400'}`}>{row.right === null ? <span className="text-zinc-400">—</span> : row.right}</div></div>)}</div></div>}
