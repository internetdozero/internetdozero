...
export function QrCodeGenerator({ lang = 'pt' }) {
  ...
  return (
    <section className="mt-10 overflow-hidden rounded-3xl border border-zinc-200 bg-white dark:border-zinc-800 dark:bg-zinc-900/60" aria-labelledby="qr-code-title">
      <div className="border-b border-zinc-200 p-5 dark:border-zinc-800 sm:p-7"><div className="flex items-start justify-between gap-4"><div><p className="font-mono text-[11px] font-bold uppercase tracking-[0.2em] text-stone-500">geração local</p><h2 id="qr-code-title" className="mt-2 font-mono text-xl font-bold text-zinc-900 dark:text-white sm:text-2xl">{isEn ? 'Styled QR code' : 'Gerador de QR Code'}</h2><p className="mt-2 text-sm leading-relaxed text-zinc-500 dark:text-zinc-400">{isEn ? 'Create a custom QR code and download it as PNG or SVG.' : 'Crie um QR Code customizado e baixe em PNG ou SVG.'}</p></div><ShieldCheck className="mt-1 h-5 w-5 shrink-0 text-stone-500" aria-label={isEn ? 'Local processing' : 'Processamento local'} /></div></div>
      <div className="grid gap-6 p-5 sm:p-7 lg:grid-cols-[1fr_300px]">
        ...
        <div className="space-y-4 rounded-2xl border border-zinc-200 bg-zinc-50 p-4 dark:border-zinc-800 dark:bg-zinc-950/50"><label className="block text-xs font-semibold text-zinc-600 dark:text-zinc-400">{isEn ? 'Text or URL' : 'Texto ou URL'}<textarea value={text} onChange={(event) => setText(event.target.value)} rows="3" placeholder={isEn ? 'https://…' : 'https://…'} className="mt-2 w-full resize-y rounded-xl border border-zinc-200 bg-white p-3 text-sm text-zinc-800 outline-none focus:border-stone-500 focus-visible:ring-2 focus-visible:ring-stone-500/30 dark:border-zinc-800 dark:bg-zinc-900 dark:text-zinc-200" /></label><div className="grid grid-cols-2 gap-3"><label className="text-xs font-semibold text-zinc-600 dark:text-zinc-400">{isEn ? 'QR color' : 'Cor do QR'}<input type="color" value={color} onChange={(event) => setColor(event.target.value)} className="mt-2 h-10 w-full cursor-pointer rounded-lg border border-zinc-200 bg-white p-1 dark:border-zinc-800 dark:bg-zinc-900" /></label><label className="text-xs font-semibold text-zinc-600 dark:text-zinc-400">{isEn ? 'Background' : 'Fundo'}<input type="color" value={background} onChange={(event) => setBackground(event.target.value)} className="mt-2 h-10 w-full cursor-pointer rounded-lg border border-zinc-200 bg-white p-1 dark:border-zinc-800 dark:bg-zinc-900" /></label></div><label className="flex items-center gap-2 text-xs font-semibold text-zinc-600 dark:text-zinc-400"><input type="checkbox" checked={rounded} onChange={(event) => setRounded(event.target.checked)} className="h-4 w-4 accent-stone-500" />{isEn ? 'Rounded modules' : 'Módulos arredondados'}</label><input ref={inputRef} type="file" accept="image/*" className="sr-only" onChange={(event) => selectLogo(event.target.files?.[0])} /><button type="button" onClick={() => inputRef.current?.click()} className="inline-flex w-full items-center justify-center gap-2 rounded-xl border border-zinc-300 px-4 py-2.5 text-sm font-bold text-zinc-700 hover:border-stone-500 hover:text-stone-600 focus-visible:outline-none focus-visible:ring-2 focus-visible:ring-stone-500 dark:border-zinc-700 dark:text-zinc-300"><ImagePlus className="h-4 w-4" />{logo ? (isEn ? 'Change logo' : 'Trocar logo') : (isEn ? 'Add center logo' : 'Adicionar logo central')}</button>{logo && <button type="button" onClick={clearLogo} className="inline-flex items-center gap-1.5 text-xs font-semibold text-zinc-500 hover:text-stone-500 focus-visible:outline-none focus-visible:ring-2 focus-visible:ring-stone-500"><RefreshCw className="h-3.5 w-3.5" />{isEn ? 'Remove logo' : 'Remover logo'}</button>}<div className="border-t border-zinc-200 pt-4 dark:border-zinc-800"><p className="mb-3 font-mono text-[10px] font-bold uppercase tracking-[0.16em] text-zinc-500">{isEn ? 'Download' : 'Baixar'}</p><div className="grid grid-cols-2 gap-2"><button type="button" disabled={!qr} onClick={() => exportQr('png')} className="inline-flex items-center justify-center gap-1.5 rounded-xl bg-stone-500 px-3 py-2.5 text-sm font-bold text-zinc-950 hover:bg-stone-400 disabled:cursor-not-allowed disabled:opacity-40"><Download className="h-4 w-4" />PNG</button><button type="button" disabled={!qr} onClick={() => exportQr('svg')} className="inline-flex items-center justify-center gap-1.5 rounded-xl border border-stone-500/40 px-3 py-2.5 text-sm font-bold text-stone-600 hover:bg-stone-500/10 disabled:cursor-not-allowed disabled:opacity-40 dark:text-stone-400"><QrCode className="h-4 w-4" />SVG</button></div>{download && <a href={download.url} download={download.name} className="mt-3 flex items-center justify-center gap-2 rounded-xl border border-zinc-300 px-3 py-2 text-xs font-bold text-zinc-600 hover:border-stone-500 hover:text-stone-600 dark:border-zinc-700 dark:text-zinc-300"><Download className="h-3.5 w-3.5" />{isEn ? `Download ${download.label}` : `Baixar ${download.label}`}</a>}</div>{error && <p role="alert" className="text-xs font-semibold text-red-500">{error}</p>}</div>
      </div>
    </section>
  );
}
