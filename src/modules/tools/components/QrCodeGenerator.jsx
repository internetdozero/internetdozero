...
export function QrCodeGenerator({ lang = 'pt' }) {
  ...
  const [text, setText] = useState('https://internetdozero.com.br');
  ...
  const qr = useMemo(() => { try { return text.trim() ? QRCode.create(text.trim(), { errorCorrectionLevel: 'H' }) : null; } catch { return null; } }, [text]);
  ...
  return (
    <section className="mt-10 overflow-hidden rounded-3xl border border-zinc-200 bg-white dark:border-zinc-800 dark:bg-zinc-900/60" aria-labelledby="qr-code-title">
      ...
      <div className="grid gap-6 p-5 sm:p-7 lg:grid-cols-[1fr_300px]">
        <div className="flex min-h-[24rem] items-center justify-center rounded-2xl border border-zinc-200 bg-zinc-100 p-6 dark:border-zinc-800 dark:bg-zinc-950/50"><div className="w-full max-w-[23rem] bg-white p-3 shadow-sm" dangerouslySetInnerHTML={{ __html: svg || '<p class="p-8 text-center text-sm text-zinc-500">Digite algo para gerar.</p>' }} /></div>
