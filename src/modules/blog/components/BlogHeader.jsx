  ...
  return (
    <header className="mb-10 pb-6 border-b border-zinc-200 dark:border-zinc-800">
      ...
      <div className="flex items-center justify-between gap-4 mb-4">
        <button
          onClick={onBackToHub}
          className="inline-flex items-center gap-2 text-xs text-zinc-500 dark:text-zinc-400 hover:text-stone-800 hover:underline cursor-pointer group"
        >
      ...
      <div className="flex flex-col md:flex-row md:items-end justify-between gap-6">
        <div>
          <h1 className="text-3xl sm:text-5xl font-serif font-semibold text-zinc-900 dark:text-white tracking-tight">
            {t.blog.title}
          </h1>
        ...
        <div className="relative w-full md:w-72">
          ...
          <input
            ...
            className="w-full pl-10 pr-4 py-2.5 rounded-xl bg-white dark:bg-zinc-900/80 border border-zinc-300 dark:border-zinc-800 text-xs font-mono focus:outline-none focus:border-stone-500 transition-colors shadow-xs"
          />
        </div>
      </div>
      ...
      <div className="mt-6 flex flex-col gap-3 border-t border-zinc-100 pt-4 dark:border-zinc-800/60 md:flex-row md:items-center md:justify-between">
          ...
          {tabs.map((tab) => (
            <button
              ...
              className={`px-4 py-1.5 rounded-xl text-xs font-mono transition-all cursor-pointer ${
                activeTab === tab.id
                  ? 'bg-zinc-900 dark:bg-stone-500 text-white dark:text-zinc-950 font-bold shadow-xs'
                  : 'bg-white dark:bg-zinc-900 border border-zinc-200 dark:border-zinc-800 text-zinc-600 dark:text-zinc-400 hover:border-stone-500/40 hover:text-zinc-900 dark:hover:text-white'
              }`}
            >
              {tab.label}
            </button>
        ...
        {categories.length > 0 && (
          <div className="relative">
            <button type="button" aria-expanded={filterOpen} onClick={() => setFilterOpen((open) => !open)} className={`inline-flex items-center gap-2 rounded-xl border px-3 py-1.5 text-xs font-mono transition-colors ${activeCategory ? 'border-stone-500/60 bg-stone-500/10 text-stone-600 dark:text-stone-400' : 'border-zinc-200 bg-white text-zinc-600 hover:border-stone-500/50 dark:border-zinc-800 dark:bg-zinc-900 dark:text-zinc-400'}`}>
              <Filter className="h-3.5 w-3.5" />
              {isEn ? 'Filter' : 'Filtrar'}
              {activeCategory && <span className="max-w-32 truncate rounded-md bg-stone-500/15 px-1.5 py-0.5 text-[10px]">{activeCategory}</span>}
            </button>
              ...
              <div className="absolute right-0 top-full z-20 mt-2 w-[min(calc(100vw-2rem),360px)] rounded-2xl border border-zinc-200 bg-white p-3 shadow-xl dark:border-zinc-800 dark:bg-zinc-900">
                <div className="flex items-center gap-2 border-b border-zinc-100 pb-3 dark:border-zinc-800">
                  <div className="relative min-w-0 flex-1">
                    ...
                    <input value={categoryQuery} onChange={(event) => setCategoryQuery(event.target.value)} placeholder={isEn ? 'Find a category…' : 'Buscar categoria…'} aria-label={isEn ? 'Find a category' : 'Buscar categoria'} className="w-full rounded-lg border border-zinc-200 bg-zinc-50 py-2 pl-9 pr-3 text-xs font-mono text-zinc-800 outline-none focus:border-stone-500 dark:border-zinc-800 dark:bg-zinc-950 dark:text-zinc-200" />
                  </div>
                ...
                <div className="mt-3 flex items-center justify-between border-t border-zinc-100 pt-3 dark:border-zinc-800">
                  <button type="button" onClick={() => { onSelectCategory(null); setCategoryQuery(''); setFilterOpen(false); }} className="text-[11px] font-mono text-zinc-500 hover:text-stone-500">{isEn ? 'Clear filter' : 'Limpar filtro'}</button>
                  <button type="button" onClick={onViewAllCategories} className="text-[11px] font-mono text-stone-600 hover:text-stone-500 dark:text-stone-400">{isEn ? 'All categories' : 'Todas as categorias'}</button>
                </div>
              </div>
            )}
          </div>
