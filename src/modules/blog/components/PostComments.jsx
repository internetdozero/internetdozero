...
export function PostComments({ comments = [], onAddComment }) {
  ...
  return (
    <section className="mt-12 pt-8 border-t border-zinc-200 dark:border-zinc-800">
      ...
      <form onSubmit={handleSubmit} className="mb-8 p-5 rounded-2xl bg-zinc-50 dark:bg-zinc-900/60 border border-zinc-200 dark:border-zinc-800">
        <div className="mb-3">
          <label className="block text-xs font-mono text-zinc-500 mb-1">Seu Nome / Apelido (opcional)</label>
          <input
            type="text"
            value={author}
            onChange={(e) => setAuthor(e.target.value)}
            placeholder="Ex: Visitante ou @seuuser"
        ...
        <div className="mb-3">
          <label className="block text-xs font-mono text-zinc-500 mb-1">Deixe seu comentário *</label>
          <textarea
            required
            rows={3}
            value={text}
            onChange={(e) => setText(e.target.value)}
            placeholder="Manda a sua opinião, sem frescura..."
        ...
        <button
          ...
          className="inline-flex items-center gap-2 px-5 py-2.5 rounded-xl bg-emerald-600 hover:bg-emerald-500 text-white font-mono font-bold text-xs transition-all disabled:opacity-50 cursor-pointer shadow-sm"
        >
      ...
      <div className="space-y-4">
        {comments.length === 0 ? (
          <p className="text-center py-6 text-sm text-zinc-500 font-mono">
            Nenhum comentário por aqui ainda. Seja o primeiro a falar alguma coisa!
          </p>
        ) : (
