...
function Login({ onLogin }) {
  ...
  const submit = (event) => {
    ...
    fetch('/api/auth/login', { method: 'POST', credentials: 'same-origin', headers: { 'Content-Type': 'application/json' }, body: JSON.stringify({ password }) }).then(async (response) => { const data = await response.json(); if (!response.ok) throw new Error(data.error || 'Falha ao entrar'); sessionStorage.setItem('idz_admin_csrf', data.csrf); onLogin(); }).catch((error) => setError(error.message));
  };
...
export function AdminView({ onNavigate }) {
  ...
  useEffect(() => { fetch('/api/auth/me', { credentials: 'same-origin' }).then((response) => { if (!response.ok) throw new Error('not-authenticated'); return response.json(); }).then((data) => { sessionStorage.setItem('idz_admin_csrf', data.csrf); setAuthenticated(true); }).catch(() => {}); }, []);
  useEffect(() => { if (authenticated) { (async () => { const legacyPosts = await blogApi.getPosts(); await fetch('/api/admin/import', { method: 'POST', credentials: 'same-origin', headers: { 'Content-Type': 'application/json', 'X-CSRF-Token': sessionStorage.getItem('idz_admin_csrf') || '' }, body: JSON.stringify(legacyPosts) }); setPosts(await blogApi.getPosts()); setCategories(await blogApi.getCategories()); })(); } }, [authenticated]);
  ...
  const createCategory = () => { const name = newCategory.trim(); if (!name || categories.includes(name)) return; fetch('/api/admin/categories', { method: 'POST', credentials: 'same-origin', headers: { 'Content-Type': 'application/json', 'X-CSRF-Token': sessionStorage.getItem('idz_admin_csrf') || '' }, body: JSON.stringify({ name }) }).then((response) => { if (!response.ok) throw new Error('Falha ao criar categoria'); const next = [...categories, name]; setCategories(next); update('category', name); setNewCategory(''); }).catch((error) => setMessage(error.message)); };
  const logout = () => { fetch('/api/auth/logout', { method: 'POST', credentials: 'same-origin', headers: { 'X-CSRF-Token': sessionStorage.getItem('idz_admin_csrf') || '' } }).finally(() => { sessionStorage.removeItem('idz_admin_csrf'); setAuthenticated(false); }); };
